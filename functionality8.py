...
def fun():
    ...
    spark.sql("create temporary view temp1 as (select Stock_names, avg(Volume) as Average_Volume from data group by Stock_names)").show()
    spark.sql("create temporary view temp2 as (select max(Average_Volume) as Average_Volume from temp1)").show()
